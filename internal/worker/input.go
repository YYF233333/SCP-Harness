...
func ValidateInput(v Input) error {
	...
	if v.Version != 0 || !idPattern.MatchString(v.AttemptID) || !operations[v.Operation] || v.OutputSchema != v.Operation || strings.TrimSpace(v.Objective) == "" || !idPattern.MatchString(v.Target.ID) || !shaPattern.MatchString(v.Origin.SHA) || v.Origin.Revision < 0 || v.Context.Root != "/scp/attempt/context" || v.Workspace.Root != "/scp/attempt/workspace" {
		return bad()
	}
